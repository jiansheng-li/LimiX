...
def optuna_inference_with_cluster(model_path,trainX, trainy, testX, testy,idx,n_trials:int=1000):
    ...
    config, result = searchInference.search(method, n_trials=n_trials, metric="precision")
    return result
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_trials', default=1000, type=int, help="number of trials")
    args = parser.parse_args()
    os.makedirs(args.save_name,exist_ok=True)
    save_result_path = os.path.join(args.save_name, "inference_with_cluster.csv")
    ...
    for idx, folder in tqdm(enumerate(os.listdir(data_root)), total=len(os.listdir(data_root)), desc=f"dataset",
        ...
        try:
            ...
            metric=optuna_inference_with_cluster(args.model_path, trainX, trainy, testX, testy,0,args.n_trials)
            rst["precision"]=metric
            rsts.append(rst)
        except Exception as e:
