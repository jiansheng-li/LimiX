...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', default="./tsne_all/tabzilla", type=str, help="path to save result")
    ...
    for idx, folder in tqdm(enumerate(os.listdir(data_root)),total=len(os.listdir(data_root)), desc=f"dataset",
        ...
        try:
            ...
            embedding_.append(simple_inference(model, trainX, trainy, trainX, "emb").clone().detach().cpu().numpy()[:len(trainy)])
        except Exception as e:
        ...
        try:
            ...
            for label in np.unique(trainy):
                ...
                if len(indices_for_label) > 0:
                    ax.scatter(X_tsne[last:last+len(trainy), 0][indices_for_label],
                               X_tsne[last:last+len(trainy), 1][indices_for_label],
                               c=[colors[idx]],
                               s=10,
                               alpha=0.9,
                               ...
                               linewidth=0.8,
                               label=f'Selected {idx} Train Label {label}')
            last=len(trainy)
        except Exception as e:
            print(f"Error processing: {e}")
            gc.collect()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f"train_all.pdf"),dpi=300,format="pdf")
    plt.close()
