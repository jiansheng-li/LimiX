...
if __name__ == '__main__':
    ...
    for idx, folder in tqdm(enumerate(os.listdir(data_root)),total=len(os.listdir(data_root)), desc=f"dataset",
        ...
        try:
            ...
            selected_marker = 'o'  # 选中的点
            unselected_marker = '^'  # 未选中的点
            ...
            for label in np.unique(trainy):
                mask = trainy == label
                indices_for_label = np.array(len(trainy))[mask]
                if len(indices_for_label) > 0:
                    ax.scatter(X_tsne[:len(trainy), 0],
                               X_tsne[:len(trainy), 1],
                               c=[color_map[label]],
                               s=30,
                               alpha=0.9,
                               ...
                               edgecolors='black',
                               linewidth=0.8,
                               label=f'Selected Train Label {label}' if idx == 0 else "")
            for label in np.unique(trainy):
                mask = testy == label
                indices_for_label = np.array(len(testy))[mask]
                if len(indices_for_label) > 0:
                    ax.scatter(X_tsne[len(trainy):, 0],
                               X_tsne[len(trainy):, 1],
                               c=[color_map[label]],
                               ...
                               edgecolors='white',
                               linewidth=0.8,
                               label=f'Selected Test Label {label}' if idx == 0 else "")
            ax.legend(loc='upper right', fontsize=8, ncol=2)
            plt.tight_layout()
            plt.savefig(os.path.join(save_path, f"{folder}_original.png"))
            plt.close()
            for i in range(20):
                ...
                tsne = TSNE(n_components=2,
                            ...
                            learning_rate='auto',
                            n_iter=1000)
                X_tsne = tsne.fit_transform(np.concatenate((embedding[train_indices],embedding[test_indices]),axis=0))
                ...
                #     if len(indices_for_label) > 0:
                #         ax.scatter(X_tsne[indices_for_label, 0],
                #                    X_tsne[indices_for_label, 1],
                #                    c=[color_map[label]],
                #                    s=30,
                #                    alpha=0.6,
                #                    edgecolors='black',
                ...
                #                    label=f'Unselected Train Label {label}' if idx == 0 else "")
                #
                #     # 绘制未选中的测试数据点（按标签着色）
                # for label in np.unique(trainy):
                ...
                #                    linewidth=0.8,
                #                    marker=unselected_marker,
                #                    label=f'Unselected Test Label {label}' if idx == 0 else"")

                for label in np.unique(trainy):
                    mask = trainy[train_indices] == label
                    indices_for_label = np.array(train_indices)[mask]
                    if len(indices_for_label) > 0:
                        ax.scatter(X_tsne[:len(train_indices), 0],
                                   X_tsne[:len(train_indices), 1],
                                   c=[color_map[label]],
                                   s=30,
                                   alpha=0.9,
                                   marker=selected_marker,
                ...
                for label in np.unique(trainy):
                    ...
                    indices_for_label = np.array(test_indices)[mask]
                    if len(indices_for_label) > 0:
                        ax.scatter(X_tsne[len(train_indices):, 0],
                                   X_tsne[len(train_indices):, 1],
                                   c=[color_map[label]],
                ...
                ax.legend(loc='upper right', fontsize=8, ncol=2)
                plt.tight_layout()
                plt.savefig(os.path.join(save_path, f"{folder}_{i}.png"))
                plt.close()
        except Exception as e:
