...
from finetuning.data_utils import TabularFinetuneDataset
...
from utils.loading import load_model, load_finetuning_model
...
def finetune(model,trainX:np.ndarray,trainy:np.ndarray):
    """
    ...
    data=TabularFinetuneDataset(trainX,trainy)
...
if __name__ == '__main__':
    ...
    for idx, folder in tqdm(enumerate(os.listdir(data_root)),total=len(os.listdir(data_root)), desc=f"dataset",
        ...
        try:
            ...
            model = load_finetuning_model(model_path)
            finetune(model)
